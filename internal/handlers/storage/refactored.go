...
import (
	...
	storageuc "motion-index-fiber/internal/application/usecase/storage"
	"motion-index-fiber/internal/models"
	"motion-index-fiber/pkg/storage"
)

// RefactoredHandler provides HTTP handlers for storage operations using DDD use cases.
type RefactoredHandler struct {
	storageService storage.Service
	listUC         *storageuc.ListStorageObjectsUseCase
	countUC        *storageuc.CountStorageObjectsUseCase
	searchUC       *storageuc.SearchStorageObjectsUseCase
	getURLUC       *storageuc.GetStorageObjectURLUseCase
}

// NewRefactoredHandler creates a new refactored storage handler with use case injection.
func NewRefactoredHandler(storageService storage.Service) *RefactoredHandler {
	return &RefactoredHandler{
		storageService: storageService,
		listUC:         storageuc.NewListStorageObjectsUseCase(storageService),
		countUC:        storageuc.NewCountStorageObjectsUseCase(storageService),
		searchUC:       storageuc.NewSearchStorageObjectsUseCase(storageService),
		getURLUC:       storageuc.NewGetStorageObjectURLUseCase(storageService),
	}
}

// ListDocuments handles GET /api/storage/documents - List documents with pagination.
func (h *RefactoredHandler) ListDocuments(c *fiber.Ctx) error {
...
func (h *RefactoredHandler) GetDocumentsCount(c *fiber.Ctx) error {
	minSize, _ := strconv.ParseInt(c.Query("min_size", "0"), 10, 64)
	var maxSize int64 = -1
	if maxSizeStr := c.Query("max_size", ""); maxSizeStr != "" {
		maxSize, _ = strconv.ParseInt(maxSizeStr, 10, 64)
...
func (h *RefactoredHandler) FindDocumentsByName(c *fiber.Ctx) error {
...
func (h *RefactoredHandler) ServeDocument(c *fiber.Ctx) error {
